...
> **Evidence notice:** reconstructed after the prototype. No test-strategy
> interview was conducted, so no row cites invented interview answers. Existing
> tests are evidence of implementation; hosted execution remains separate.
...
Hot-spot scope used for likelihood weighting: the learning flow introduced in
the single MVP commit spans `src/lib/`, `src/pages/api/`,
`src/components/dashboard/`, `supabase/migrations/` and `e2e/`. The repository
has insufficient post-MVP churn history for statistical hot-spot ranking; impact
and boundary count therefore carry more weight than commit frequency.
...
Because implementation preceded this plan, one reconstructed change folder
contains the historical MVP work. Future rollouts should use one distinct
change folder per row.

| #   | Phase name                    | Goal (one line)                                                              | Risks covered | Test types              | Status       | Change folder                             |
| --- | ----------------------------- | ---------------------------------------------------------------------------- | ------------- | ----------------------- | ------------ | ----------------------------------------- |
| 1   | Domain and boundary contracts | Make incorrect scoring and invalid writes fail cheaply.                      | #1, #5        | unit + contract         | implementing | `context/changes/ai-concept-compass-mvp/` |
...
Status remains `implementing` where automated artifacts exist but the matching
manual/hosted proof in the change plan is still unchecked.
...
| database/RLS integration | Supabase JS against two ordinary hosted accounts     | 2.99.1  | Implemented harness; hosted execution remains pending credentials.     |
