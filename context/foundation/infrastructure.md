...
> **Evidence notice:** this decision record was reconstructed after the prototype
> existed. It is not evidence that the five-question `/10x-infra-research`
> interview was conducted before implementation. Scores combine the supplied
> product constraints, current repository configuration and official platform
> documentation checked on 2026-07-31. Unknown preferences remain explicit.

## Recommendation

**Deploy on Cloudflare Workers.**

The repository already uses the official Cloudflare adapter, SSR output and a
Workers entrypoint, so this option avoids an adapter migration and keeps the
runtime aligned with local and production builds. At the expected MVP scale the
